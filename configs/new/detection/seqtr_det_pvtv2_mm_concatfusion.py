...
model = dict(
    ...
    lan_enc=dict(
        ...
        freeze_emb=True,
        output_cfg=dict(type="original"),
    ),
    fusion=dict(type="MDETRFusion", vis_chs=[128, 320, 512], fusion_dim=fusion_dim, direction="bottom_up"),
    head=dict(
        type="MDETRHead",
        in_ch=fusion_dim,
        num_bin=num_bin,
        ...
        transformer=dict(
            ...
            encoder=dict(
                fusion_dim=fusion_dim, num_layers=6, layer=dict(d_model=d_model, nhead=8, dim_feedforward=4 * d_model, dropout=0.1, activation="relu", batch_first=True)
            ),
            decoder=dict(
                num_layers=3,
