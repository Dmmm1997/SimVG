...
class TextGuidedQuerySelectDETRHead(nn.Module):
    def __init__(
        self,
        num_queries=100,
        in_channels=768,
        ...
        super(TextGuidedQuerySelectDETRHead, self).__init__()
        ...
        self.query_proj = nn.Linear(text_max_token, num_queries)
    ...
    def forward_train(
        ...
        # query_embed = (
        #     self.query_embed.weight.unsqueeze(0).repeat((text_feat.shape[0], 1, 1)) + torch.max(text_feat, dim=1, keepdim=True)[0]
        # )
        query_embed = self.query_proj(text_feat.transpose(1, 2)).transpose(1, 2)
