...
class SEQHead_ConcatAttention(nn.Module):
    def __init__(self,
                 in_ch=1024,
                 num_bin=1000,
                 multi_task=False,
        ...
        super(SEQHead_ConcatAttention, self).__init__()
    ...
    def forward_train(self,
        ...
        """
        x_feat = x_mm["vision_feat"]
        y_feat = x_mm["text_feat"]
        y_mask = x_mm["text_mask"]
        
        with_bbox = gt_bbox is not None
        with_mask = gt_mask_vertices is not None

        x_mask, x_pos_embeds = self.transformer.x_mask_pos_enc(x_feat, img_metas)

        memory, mask, pos_embeds = self.transformer.forward_encoder_concat(x_feat, x_mask, x_pos_embeds, y_feat, y_mask)
        ...
        logits = self.transformer.forward_decoder(
            seq_in_embeds, memory, pos_embeds, mask)
    ...
    def forward_test(self, x_mm, img_metas, with_bbox=False, with_mask=False):
        ...
        x_mask, x_pos_embeds = self.transformer.x_mask_pos_enc(x_feat, img_metas)
        memory, mask, pos_embeds = self.transformer.forward_encoder_concat(x_feat, x_mask, x_pos_embeds, y_feat, y_mask)
        return self.generate_sequence(memory, mask, pos_embeds, 
                                      with_bbox=with_bbox, 
                                      with_mask=with_mask)
