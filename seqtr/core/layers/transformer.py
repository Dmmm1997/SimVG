...
class AutoRegressiveTransformer(BaseModule):
    ...
    def forward_decoder(self,
        ...
        query_in_mask = self.tri_mask(
            query_in_embeds.size(1)).to(query_in_embeds.device)

        tgt = self.decoder(query_in_embeds, memory,
                           pos=x_pos_embeds,
                           query_pos=query_in_pos_embeds,
                           memory_key_padding_mask=x_mask,
                           tgt_mask=query_in_mask)
        return tgt
    
    def forward_decoder_mdetr(self,
                        seq_in_embeds,
                        memory,
                        x_pos_embeds,
                        x_mask):
