...
from simvg.core.layers import LinearModule
from simvg.core.losses import LabelSmoothCrossEntropyLoss
...
class SeqHead(nn.Module):
    def __init__(
        ...
        seq_positional_encoding=dict(
            type="LearnedPositionalEncoding1D", num_embedding=5, num_feature=256
        ),
    ):
        ...
        self.onlydecoder = onlydecoder

        self.transformer = build_transformer(transformer)
        self.d_model = self.transformer.d_model

        self._init_layers(
            in_ch, predictor, multi_task, x_positional_encoding, seq_positional_encoding
        )
    ...
    def quantize(self, seq, img_metas):
        if self.mapping == "relative":
            ...
            norm_factor = [img_meta["pad_shape"][:2][::-1] for img_meta in img_metas]
            norm_factor = seq.new_tensor(norm_factor)
            norm_factor = torch.cat([norm_factor for _ in range(num_pts)], dim=1)
            return (seq / norm_factor * self.num_bin).long()
        elif self.mapping == "absolute":
            return (seq / 640.0 * self.num_bin).long()

    def target_norm(self, target, img_metas):
        target = torch.stack(target, dim=0)
        norm_bbox = torch.zeros_like(target, device=target.device)
        norm_bbox[:, 0] = (target[:, 0] + target[:, 2]) / 2.0  # x_center
        norm_bbox[:, 1] = (target[:, 1] + target[:, 3]) / 2.0  # y_center
        norm_bbox[:, 2] = target[:, 2] - target[:, 0]  # w
        norm_bbox[:, 3] = target[:, 3] - target[:, 1]  # h
        num_pts = norm_bbox.size(1) // 2
        norm_factor = [img_meta["pad_shape"][:2][::-1] for img_meta in img_metas]
        norm_factor = norm_bbox.new_tensor(norm_factor)
        norm_factor = torch.cat([norm_factor for _ in range(num_pts)], dim=1)
        return norm_bbox / norm_factor

    def dequantize(self, seq, scale_factor):
        if self.mapping == "relative":
            return seq * scale_factor / self.num_bin
        elif self.mapping == "absolute":
            return seq * 640.0 / self.num_bin
    ...
    def sequentialize(
        ...
        if with_bbox and with_mask:
            ...
            targets = torch.cat(
                [
                    seq_in_bbox,
                    seq_in_bbox.new_full((batch_size, 1), self.end),
                    seq_in_mask,
                    seq_in_mask.new_full((batch_size, 1), self.end),
                ],
                dim=-1,
            )
            seq_in_embeds_bbox = self.transformer.query_embedding(seq_in_bbox)
            seq_in_embeds_mask = self.transformer.query_embedding(seq_in_mask)
            task_bbox = (
            ...
            if with_mask and self.shuffle_fraction > 0.0:
            ...
            seq_in_embeds = torch.cat(
                [seq_in_embeds.new_zeros((batch_size, 1, self.d_model)), seq_in_embeds],
                dim=1,
            )
            return seq_in_embeds, targets

    def forward_train(self, x_mm, img_metas, gt_bbox=None, gt_mask_vertices=None):
        """Args:
        ...
        if not self.onlydecoder:
            memory = self.transformer.forward_encoder(x_mm, x_mask, x_pos_embeds)
        else:
            x_mm = self.transformer.input_proj(x_mm)
            memory = x_mm.view(x_mm.size(0), self.d_model, -1).transpose(1, 2)

        seq_in_embeds, targets = self.sequentialize(
            img_metas, gt_bbox=gt_bbox, gt_mask_vertices=gt_mask_vertices
        )
        ...
        if self.loss_bbox is not None:
            targets = self.target_norm(gt_bbox, img_metas)
            loss_bbox = self.loss(logits, targets)
