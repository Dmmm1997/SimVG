...
class SeqTR(OneStageModel):
    ...
    def forward_train(self, img, ref_expr_inds, img_metas, gt_bbox=None, gt_mask_vertices=None, mass_center=None, gt_mask=None, rescale=False):
        ...
        """
        x, y = self.extract_visual_language(img, img_metas, ref_expr_inds)

        x_mm = self.fusion(x, y)
        losses_dict, seq_out_dict = self.head.forward_train(x_mm, img_metas, gt_bbox=gt_bbox, gt_mask_vertices=gt_mask_vertices)
        with torch.no_grad():
    ...
    def forward_test(self, img, ref_expr_inds, img_metas, with_bbox=False, with_mask=False, rescale=False):
        ...
        seq_out_dict = self.head.forward_test(x_mm, img_metas, with_bbox=with_bbox, with_mask=with_mask)
    ...
    def get_predictions(self, seq_out_dict, img_metas, rescale=False):
        ...
        if with_mask:
            ...
            for pred_mask, img_meta in zip(seq_out_mask, img_metas):
                ...
                ends = numpy.argwhere(pred_mask == self.head.end)[:, 0]
                if not len(ends) == 0:
                    pred_mask = pred_mask[: ends[0]]
                if len(pred_mask) % 2 != 0:  # must be even
                    pred_mask = pred_mask[:-1]

                scale_factor = [[w_pad, h_pad] for _ in range(len(pred_mask) // 2)]
                ...
                pred_rles = maskUtils.frPyObjects(pred_mask, h_pad, w_pad)  # list[rle]
        ...
        if with_bbox:
            ...
            scale_factor = [img_meta["pad_shape"][:2][::-1] for img_meta in img_metas]
