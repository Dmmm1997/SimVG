...
def box_seg_visualization(pred_box, pred_seg, save_filename, img_metas, text, gt_mask, gt_box, threshold=0.5):
    ...
    draw_pred.rectangle(list(pred_box), outline="red", width=2)
    ...
    concat_image = Image.new("RGB", (W * 3 + 20, H))
    concat_image.paste(img_source, (0, 0))
    concat_image.paste(image_gt, (W + 10, 0))
    concat_image.paste(image_pred, (W * 2 + 20, 0))
...
class MIXUniModel(OneStageModel):
    ...
    def forward_train(
        ...
        losses_dict, pred_dict = self.head.forward_train(img_feat, targets, cls_feat, text_feat, text_attention_mask, img)

        with torch.no_grad():
            predictions = self.get_predictions(pred_dict, img_metas, rescale=rescale, threshold=self.threshold)

        self.iter += 1
        if is_main() and self.iter % 50 == 0:
            self.visualiation(pred_dict, img_metas, targets, self.train_mask_save_target_dir)

        return losses_dict, predictions

    def visualiation(self, pred_dict, img_metas, targets, save_target_dir):
    ...
    def forward_test(
        ...
        pred_dict = self.head.forward_test(img_feat, cls_feat, text_feat, text_attention_mask, img, targets=targets)

        predictions = self.get_predictions(pred_dict, img_metas, rescale=rescale, threshold=self.threshold)

        self.iter += 1
        if is_main() and self.iter % 20 == 0:
            self.visualiation(pred_dict, img_metas, targets, self.val_mask_save_target_dir)
    ...
    def get_predictions(self, pred, img_metas, rescale=False, threshold=0.5):
        ...
        if mask_seg is not None:
            ...
            mask_binary[mask_binary<threshold] = 0.
            mask_binary[mask_binary>=threshold] = 1.
