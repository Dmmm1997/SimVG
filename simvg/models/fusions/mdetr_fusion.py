...
from simvg.models import FUSIONS
from simvg.models.vis_encs.darknet import darknet_conv


@FUSIONS.register_module()
class MDETRFusion(nn.Module):
    def __init__(self,
                 vis_chs=(256, 512, 1024),
                 direction='bottom_up',
                 fusion_dim=1024,
                 ):
        super(MDETRFusion, self).__init__()
    ...
    def forward(self, x, y):
        ...
        text_mask = y["text_mask"]

        text_feat = self.activate(text_feat)
        x_vis_enc = self.activate(self.linear(x_vis_enc))
        res = {
            "vision_feat": x_vis_enc*text_feat.mean(dim=1).unsqueeze(-1).unsqueeze(-1),
            # "vision_feat": x_vis_enc,
            "text_feat": text_feat,
            "text_mask": text_mask
        }
