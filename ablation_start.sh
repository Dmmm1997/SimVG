source_dir=configs/segmentation/ablation_setting_select/contrastive-conv
file_names=$(ls $source_dir)
for file_name in $file_names
do
  related_filename=$source_dir/$file_name
  # train
  CUDA_VISIBLE_DEVICES=0,1 PORT=29501 bash tools/dist_train.sh $related_filename 2
  ...
  echo $checkpoint
  CUDA_VISIBLE_DEVICES=0,1 PORT=29511 bash tools/dist_test.sh $related_filename 2 --load-from $checkpoint
  # test -----
done
